...
@php
  // Unique, but stable per render (prevents collisions if navbar appears twice)
  $menuId = 'mobile-menu-' . uniqid();
@endphp

<nav id="lbs-nav" {{ $attributes->merge(['class' => "w-full transition-colors duration-300"]) }}>
      ...
      <div class="hidden md:flex items-center gap-8">
        @foreach($parentLeagues as $league)
          ...
          <a href="{{ route('lbs.league.show', $league->id) }}"
             class="relative font-medium transition group {{ $active ? 'text-[#84CC16]' : 'text-[#F3F4F6]/90 hover:text-[#84CC16]' }}">
            {{ $league->name }}
