{{-- resources/views/nba/compare.blade.php --}}
<!DOCTYPE html>
<html lang="lv">
<head>
  <meta charset="UTF-8" />
  <title>Players Explorer & Compare</title>
  ...
  <style>
    ...
    /* shimmer for loading blocks (used in compare spinner backdrop) */
...
<body class="bg-[#111827] text-[#F3F4F6] antialiased">
  ...
  <script>
    // Quick search (works for table view; cards rely on browser find or filters)
    ...
    // Leader-only comparison helpers
    ...
    compareBtn.addEventListener('click', () => {
      ...
      // show loading shimmer briefly to smooth big DOM inserts
      compareLoading.classList.remove('hidden');
      compareArea.classList.add('hidden');

      // small timeout to let loading UI render
      setTimeout(() => {
        ...
        compareGrid.innerHTML = sel.map((p,i) => {
          const head = p.headshot
            ? `<img src="${p.headshot}" class="h-7 w-7 rounded-full object-cover ring-1 ring-white/10 img-fade loaded" />`
            : `<div class="h-7 w-7 rounded-full bg-white/10"></div>`;
          const logo = p.logo
            ? `<img src="${p.logo}" class="h-6 w-6 object-contain rounded bg-white p-[2px] img-fade loaded" />`
            : '';
      ...
      }, 150); // short delay for nicer UX
    });

    // Intersection fade for initial cards (mobile)
