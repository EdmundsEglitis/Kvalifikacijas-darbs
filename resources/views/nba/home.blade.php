...
@section('content')
  {{-- HERO (use your stored image; bias position to top so heads aren’t cropped) --}}
      <br>
    <br>
  <br>

  <section class="relative -mt-16 w-full h-[64vh] sm:h-[70vh]">
            ...
            <article class="bg-[#0f172a] border border-[#1f2937]/70 rounded-2xl p-5 shadow hover:shadow-xl transition">
              ...
              <div class="mt-3 flex items-center justify-between gap-3">
                <div class="flex items-center gap-2 min-w-0">
                  <div class="h-8 w-8 rounded bg-white/90 grid place-items-center overflow-hidden p-1">
                    @if($g->home_team_logo)
  ...
  <section>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      @foreach($topPpg as $row)
        <a
          href="{{ route('nba.player.show', $row->player_external_id) }}"
          class="group block bg-[#0f172a] border border-[#1f2937]/70 rounded-2xl p-5 shadow
                 ...
                 focus-visible:ring-[#84CC16]/40"
          aria-label="Open {{ $row->player_name ?? 'player' }} profile"
        >
          <div class="flex items-center gap-3">
            <div
              class="h-10 w-10 rounded-full bg-white/10 grid place-items-center overflow-hidden
                     ring-0 transition group-hover:ring-2 group-hover:ring-[#84CC16]/50"
            >
          ...
          <div class="mt-4 flex items-end gap-2">
            <div
              class="text-3xl font-extrabold text-[#84CC16] transition-transform duration-300
...
<tbody class="divide-y divide-[#1f2937] bg-[#0b1220]">
  @foreach($standings as $s)
    <tr
      class="group cursor-pointer transition hover:bg-[#0f172a] focus-within:bg-[#0f172a]"
      data-href="{{ route('nba.team.show', $s->team_id) }}"
      tabindex="0"
    >
      <td class="relative px-4 py-3">
        {{-- left hover bar (works for every row) --}}
        <span
          class="pointer-events-none absolute inset-y-0 left-0 w-1 rounded-r bg-[#84CC16]/70
                 opacity-0 group-hover:opacity-100 transition"
          aria-hidden="true"></span>

        <div class="flex items-center gap-3">
          <span
            class="relative h-8 w-8 rounded bg-white/95 grid place-items-center overflow-hidden p-1
                   ring-0 transition group-hover:ring-2 group-hover:ring-[#84CC16]/40">
            @if($s->team_logo)
              <img src="{{ $s->team_logo }}"
          ...
          <a href="{{ route('nba.team.show', $s->team_id) }}"
             class="font-medium text-white transition-colors group-hover:text-[#84CC16]">
            {{ $s->team_name }}
          </a>

          {{-- chevron that slides in on hover --}}
          <svg class="ml-auto opacity-0 -translate-x-1 transition-all duration-200 ease-out
      ...
      <td class="px-4 py-3 text-right tabular-nums">
        {{ $s->avg_points_for !== null ? number_format($s->avg_points_for,1) : '—' }}
      </td>
      <td class="px-4 py-3 text-right tabular-nums">
        {{ $s->avg_points_against !== null ? number_format($s->avg_points_against,1) : '—' }}
      </td>
      <td class="px-4 py-3 text-right tabular-nums">
        <span class="transition-colors {{ ($s->point_differential ?? 0) >= 0 ? 'text-[#84CC16]' : 'text-[#F97316]' }}">
      ...
      <section>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-5">
          @foreach($teams as $t)
            @php
              $logo = $t->logo;
              // If it's an ESPN combiner URL, make it "fit" and raise the resolution
              if ($logo && strpos($logo, 'combiner/i?img=') !== false) {
                $logo = preg_replace('/w=\d+/', 'w=200', $logo);     // was 80
                $logo = preg_replace('/h=\d+/', 'h=200', $logo);     // was 80
            ...
            <a href="{{ route('nba.team.show', $t->external_id) }}"
              ...
              {{-- Logo box: taller, no overflow clipping, real contain --}}
              <div class="h-20 sm:h-24 w-full rounded bg-white/95 flex items-center justify-center">
                @if($logo)
                  <img
                    src="{{ $logo }}"
                    alt="{{ $t->name }} logo"
                    class="block max-h-16 sm:max-h-20 w-auto object-contain"
                  >
                @endif
              </div>
