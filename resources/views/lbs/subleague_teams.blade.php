...
<head>
    ...
    <title>{{ $subLeague->name }} - Teams</title>
...
<body class="bg-gray-100">
    ...
    <main class="pt-32 max-w-7xl mx-auto px-4">
        <h1 class="text-3xl font-bold text-gray-800">{{ $subLeague->name }} - Komandas</h1>

        @if($teams->isEmpty())
            <p class="mt-4 text-gray-500">Nav pievienotu komandu šai apakšlīgai.</p>
        @else
            <ul class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-4">
                @foreach($teams as $team)
                    <li class="p-4 bg-white shadow rounded-lg text-center font-medium">
                        {{ $team->name }}
                    </li>
                @endforeach
            </ul>
